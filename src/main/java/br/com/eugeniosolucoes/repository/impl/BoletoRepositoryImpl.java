    ...
    public DadosBoletoPagoModel retornarBoletoPago( String numeroBoleto ) {
        ...
        try {
            ...
            ps = con.prepareStatement( "SELECT \n"
                    ...
                    + "INNER JOIN `PFisicas` pf \n"
                    + "ON pf.`Codigo_PFisica` = mens.`Codigo_PFisica`\n"
                    + "WHERE mens.`Nosso_Numero` = ?" );
            ps.setString( 1, numeroBoleto );
            rs = ps.executeQuery();
            List<DadosBoletoPagoModel> list = new ArrayList<>();
            while (rs.next()) {
                list.add( new DadosBoletoPagoModel(
                        rs.getString( "Codigo_PFisica" ),
        ...
        try {
            ...
            while (rs.next()) {
                list.add( new DadosBoletoModel(
                        rs.getString( "CPF_PFisica" ),
                        rs.getString( "Codigo_PFisica" ),
                        MyStrings.removerAcentos( rs.getString( "Nome_PFisica" ) ),
                        null,
                        rs.getString( "Nosso_Numero" ),
                        rs.getDouble( "Valor_Baixa" ),
                        rs.getDate( "Data_Pagamento" ),
                        ...
                        null,
                        rs.getDouble( "Percentual_Multa" ),
                        rs.getDouble( "Percentual_Juros" ), null, MyStrings.removerAcentos( rs.getString( "Email_Site" ) ) ) );
            }
            return list;
        } catch ( Exception ex ) {
            throw new IllegalStateException( ex );
